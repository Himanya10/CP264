...
static void avl_rebalance(avl_node **node)
{
    ...
    int balanced = avl_balance(*node);

    // if balanced > 1 then left heavy
    if (balanced > 1)
    {
    ...
    // If balanced < -1 then right heavy
    else if (balanced < -1)
    {
...
static bool avl_insert_aux(avl_linked *source, avl_node **node, const data_ptr item)
{
    ...
    // base on the result to decide where to insert
    if (cmp < 0)
    {
        bool inserted = avl_insert_aux(source, &(*node)->left, item);
        if (inserted)
        {
            avl_update_height(*node);
            avl_rebalance(node);
    ...
    {
        // insert to right
        bool inserted = avl_insert_aux(source, &(*node)->right, item);
        if (inserted)
        {
            avl_update_height(*node);
            avl_rebalance(node);
